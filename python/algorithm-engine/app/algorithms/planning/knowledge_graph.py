...
class KnowledgeGraphPlanner:
    ...
    def plan(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        matched_rules = self._match_rules(
            all_rules, obstacles, weather, query_context, rows, cols,
        )
        reasoning_chain.append({
            "step": 2,
            "type": "rule_matching",
        ...
        path = self._plan_with_knowledge(
            start_grid, goal_grid, rows, cols, obstacles, cost_modifiers,
        )
        reasoning_chain.append({
            "step": len(reasoning_chain) + 1,
            "type": "planning",
        ...
        for i in range(len(path) - 1):
            path_cost += float(np.linalg.norm(
                np.array(path[i + 1]) - np.array(path[i]),
            ))

        logger.info(
            "知识图谱规划完成: 路径代价=%.2f, 置信度=%.4f, 应用规则=%d条",
            path_cost, overall_confidence, len(applied_rules),
        )
