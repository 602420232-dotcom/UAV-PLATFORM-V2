...
| 组件 | 最低版本 | 推荐版本 | 说明 |
|------|---------|---------|------|
| JDK | 21+ | Eclipse Temurin 21 | 主项目 Spring Boot 4.1.0 |
...
V2 支持 HashiCorp Vault 作为密钥管理后端。在 `application.yml` 中启用：
...
| 密钥路径 | 说明 | 包含字段 |
|----------|------|---------|
| `secret/uav-platform/prod/jwt` | JWT 签名密钥 | `secret` |
| `secret/uav-platform/prod/db` | 数据库密码 | `password` |
| `secret/uav-platform/prod/redis` | Redis 密码 | `password` |
| `secret/uav-platform/prod/kafka` | Kafka SASL 密码 | `password` |
| `secret/uav-platform/prod/utm` | UTM 回调密钥 | `secret` |

### 9.3 本地开发（文件回退）

开发环境可禁用 Vault，使用本地文件：
...
密钥将从 `application.yml` 或环境变量读取。

### 9.4 K8s 部署（Vault Agent Sidecar）
...
vault:
  enabled: true
  role: uav-platform
  serviceAccount: uav-platform-sa
```

Vault Agent 将以 Sidecar 形式注入，自动将密钥挂载为文件。
